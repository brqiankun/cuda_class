...
__global__ void sumArraysGPU(float* a, float* b, float* res, int offset, int n) {
    int i = blockIdx.x * blockDim.x*4 + threadIdx.x;   //0~nElem  每个thread计算连续的4次
    int k = i + offset;
    if(k+3*blockDim.x<n) {
        res[i] = a[k] + b[k];
...
int main(int argc, char* argv[]) {
    ...
    printf("exectution configuration<<<%d, %d>>> Time elapsed %f sec --offset:%d \n",grid.x, block.x, iElaps, offset);
